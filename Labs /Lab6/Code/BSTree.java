...
public class BSTree
{
   ...
   {
       if(isEmpty()){return 0;}
       return root.retrieveDepth(target); //!D
   }
   ...
   public int sum()
   {
      // iterate over the list to add integers together
      if(isEmpty()){return 0;}
      else{
         List<Integer> list = toList();
         int total = 0;
   ...
   {
      if(isEmpty() && that.isEmpty()){return true;} // two empty trees
      else if(isEmpty() || that.isEmpty()){return false;} // one empty tree
      else{ // two nonempty trees
